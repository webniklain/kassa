...
function calculateMonthlySummary(
  ...
  // Дневная рекомендация показывает реальную текущую ликвидность:
  // бюджет + приходы - все уже совершённые расходы + компенсируемые расходы.
  // Будущие обязательные платежи здесь НЕ резервируются заранее — их план/факт
  // и оставшийся резерв контролируются отдельно в настройках бюджета.
  // Когда обязательный платёж фактически совершается, он естественно уменьшает
  // текущий баланс, но до этого момента не занижает ежедневный ориентир.
